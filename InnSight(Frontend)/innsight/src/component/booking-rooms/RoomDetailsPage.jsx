...
export default function RoomDetailsPage() {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          const userProfile = await ApiService.getLoggedInUser();
    ...
    const acceptBooking = async () => {
      try {
        ...
        // Make booking
        const response = await ApiService.bookARoom(roomId, userId, booking);
        if (response.statusCode === 200) {
          setConfirmationCode(response.bookingConformationCode); // Set booking confirmation code
